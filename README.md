...
- `DEFAULT_WORKSPACE_ACCESS_CODE` should be set to a private value in the
  ...
  published** identifier `nadya-cyprus60-default-workspace-v1` so a fresh checkout
  works out of the box; `/api/health` warns while that fallback is in use.
  The fallback carries the `nadya-` prefix so this edition and any other edition
  of the app cannot land on the same profile if they are ever pointed at one
  database. Setting the env var per service is still the correct fix.
...
### Migrating an existing private-link draft into the default profile

This edition previously stored trips per browser, under a private workspace key
kept in a cookie. Those drafts still live in their own workspace rows and are
**not** what the new shared URL shows until they are copied across. If a draft
needs carrying over, run this once after deploying:

```bash
BASE=<this service's public URL>
TOKEN=<the RECOVERY_TOKEN value>
